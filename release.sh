...
# ─── Update pom.xml ───────────────────────────────────────────────────────────
# plugin.yml uses ${project.version} with Maven resource filtering — no manual
# update needed, the version is substituted automatically at build time.
...
JAR="target/happypet-${NEW_VERSION}.jar"
if [[ ! -f "$JAR" ]]; then
